...
/// <remarks>
///     <para>
///         Your app must be configured for OAuth. In you app package's <c>Package.appxmanifest</c> under Declarations, add
///         a
...
{
    private static readonly WebAuthenticator Instance = new();
    ...
    /// <summary>
    ///     Begin an authentication flow by navigating to the specified url and waiting for a callback/redirect to the
    ///     callbackUrl scheme.
    /// </summary>
    ...
    {
        return Instance.Authenticate(authorizeUri, callbackUri);
    }
    ...
    private static NameValueCollection? GetState(IProtocolActivatedEventArgs protocolArgs)
    {
        ...
        try
        {
            vals = HttpUtility.ParseQueryString(protocolArgs.Uri.Query);
        }
        catch
        {
        }
    ...
    private static void OnAppCreation()
    {
        ...
        {
            var instance = AppInstance.GetInstances().Where(i => i.Key == id).FirstOrDefault();
    ...
    private async Task<WebAuthenticatorResult> Authenticate(Uri authorizeUri, Uri callbackUri)
    {
        ...
        var state = $"appInstanceId={AppInstance.GetCurrent().Key}&signinId={g}";
